...
**Flux de connexion** : vérification du mot de passe → si MFA activé, stockage de `mfa_user_id` en session et redirection vers `/auth/mfa/verify` → `login_user()` complet uniquement après validation du MFA.
...
Les codes de secours sont stockés en JSON dans une colonne texte (`backup_codes_json`) sous forme de tableau de hashes bcrypt. Utiliser un code l'invalide en mettant son emplacement à `null`.
